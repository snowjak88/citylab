...
/**
 * This is a settings dialog, which can be shown in any view by using
 * "show:settings" LML action or - in Java code - through
 * InterfaceService.showDialog(Class) method. Thanks to the fact that it
 * implements ActionContainer, its methods will be available in the LML
 * template.
 */
@ViewDialog(id = "settings", value = "ui/templates/dialogs/settings.lml")
public class SettingsController implements ActionContainer {
	
	// @Inject-annotated fields will be automatically filled with values from the
	// context.
	@Inject
	...
	/**
	 * @param actor
	 *            requested scale change. Its ID represents a GUI scale.
	 */
